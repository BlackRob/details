...
const DrawWorkingRowContent = ({ wR, removeFromWR, updateState, cards,
  toggleWorking, undo, undoable, undoSecondsLeft, winner, accept,
  switchPlacesWR, totalCardCount, sentenceUpdateCount, gameMode }) => {
  let content = "";
  let rando = randomInterjection();

  if (undoable && gameMode !== "creative") {
    content = <KeepOrUndo undo={undo} accept={accept} undoSecondsLeft={undoSecondsLeft} />
  } else if (winner) {
...
const KeepOrUndo = ({ undo, accept, undoSecondsLeft }) => {
  return <div className="undoOrAccept">
    <button onClick={(e) => {
      e.preventDefault();
      undo();
    }}>Undo<br />{undoSecondsLeft}</button>
    <button onClick={(e) => {
      e.preventDefault();
      accept();
    }}>Accept<br />new sentence</button>
    <style jsx>{`
      .undoOrAccept {
        position: absolute;
        z-index: 1;
...
const DrawCreativeButton = ({ type, addToWR, cards, toggleWorking, inc, dec }) => {
  ...
  return <div className={`creativeButton ${activeClass}`}>
    ...
    <button className={`downButt ${downArrowStyle}`} onClick={() => dec(type)}>
...
/////////////////////// the whole shebang
const DrawCards = ({ cards, onEdit, gameMode, wR, toggleWorking, addToWR,
  removeFromWR, switchPlacesWR, undo, undoable, undoSecondsLeft,
  winner, accept, totalCardCount, sentenceUpdateCount, cardInc, cardDec }) => {
  return <div className="card_row">
    <div className="working_row">
      <div className="working_row_slot">
        <DrawWorkingRowContent wR={wR} removeFromWR={removeFromWR} updateState={onEdit} gameMode={gameMode}
          cards={cards} toggleWorking={toggleWorking} undo={undo} undoable={undoable} accept={accept}
    ...
    <div className="available_cards">
      <DrawAvailableCards wR={wR} addToWR={addToWR} cards={cards}
        toggleWorking={toggleWorking} gameMode={gameMode}
        cardInc={cardInc} cardDec={cardDec} />
    </div>
    <style jsx>
      {`
       .card_row {
